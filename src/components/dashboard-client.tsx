            ...
            <Card>
                ...
                <CardContent>
                    <ChartContainer config={churchMembershipChartConfig} className="h-[350px] w-full">
                        <BarChart accessibilityLayer data={churches} layout="vertical" margin={{ left: 120, top: 20, right: 20, bottom: 20 }}>
                            <CartesianGrid horizontal={false} />
                            <XAxis type="number" />
                            <YAxis dataKey="name" type="category" tickLine={false} axisLine={false} tickMargin={10} width={120} interval={0} />
                            <ChartTooltip
                                cursor={false}
                                content={<ChartTooltipContent indicator="dot" />}
                            />
                            <Bar dataKey="members" layout="vertical" fill="hsl(var(--primary))" radius={4}>
                               {churches.map((entry, index) => (
                                <text key={`bar-label-${index}`} x={25} y={index * 35 + 30} fill="white" textAnchor="middle">
                                    {entry.members}
                                </text>
                                ))}
                            </Bar>
